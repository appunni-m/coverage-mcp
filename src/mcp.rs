...
Every successful response is {context,data,page}; max_words is the per-call response budget (50–5000, default 600) and collections continue with page.next_cursor. Omit detailed or keep it false for normal work; set it true only when a tool description names required audit or raw-provenance fields. detailed never returns logs. The daemon uses stateless JSON responses, bounded MCP concurrency, bounded HTTP/DuckDB deadlines, and bounded HTTP bodies. A stdio bridge automatically recreates a crashed daemon after a refused connection and safely replays that undelivered request once; after an ambiguous timeout, retry an individual request with backoff and the same idempotency key. Keep query fan-out bounded. Coverage ingestion is capped at 64 MiB and malformed numeric report fields are validation errors, never silent zeroes."#
);

...
def _create_legacy_mcp(store: CoverageStore) -> FastMCP:  # pragma: no cover - removed public v0.7 contract
    mcp = FastMCP(
        "coverage-mcp",
        instructions=(
...
<body>
  ...
  <script>
    ...
    async function getJSON(url, options) {
      ...
      if (!payload || payload.context?.schema_revision !== 7 || !Object.hasOwn(payload, 'data')) {
        throw new Error('Coverage MCP dashboard requires schema revision 7. Restart the daemon after upgrading.');
      }
      return payload.data;
    }

    async function getAllJSON(url) {
      const items = [];
      let cursor = null;
      do {
        ...
        if (!payload || payload.context?.schema_revision !== 7 || !Array.isArray(payload.data)) {
          throw new Error('Coverage MCP dashboard received an invalid schema revision 7 collection.');
        }
    ...
    async function getAllFileJSON(url) {
      let file = null;
      const lines = [];
      let cursor = null;
      do {
        const pageUrl = cursor
          ...
          : url;
        const payload = await fetchJSON(pageUrl);
        if (!payload || payload.context?.schema_revision !== 7 || !payload.data?.file || !Array.isArray(payload.data.lines)) {
          throw new Error('Coverage MCP dashboard received an invalid schema revision 7 file response.');
        }
        file = payload.data.file;
        lines.push(...payload.data.lines);
        cursor = payload.page?.next_cursor || null;
      } while (cursor);
      return {file, lines};
    }

    async function getAllSectionJSON(url, section) {
      const items = [];
      let cursor = null;
      do {
        const pageUrl = cursor
          ...
          : url;
        const payload = await fetchJSON(pageUrl);
        if (!payload || payload.context?.schema_revision !== 7 || !Array.isArray(payload.data?.[section])) {
          throw new Error(`Coverage MCP dashboard received an invalid schema revision 7 ${section} response.`);
        }
        items.push(...(payload.data?.[section] || []));
        cursor = payload.page?.next_cursor || null;
      } while (cursor);
      return items;
    }
    ...
    async function refresh() {
      state.projects = await getAllJSON('/api/projects?max_words=5000');
      if (!state.projects.some(project => project.repo_key === state.projectKey)) {
        state.projectKey = state.projects.find(project => Number(project.snapshot_count) > 0)?.repo_key
          || state.projects[0]?.repo_key
          || null;
      }
      if (!state.projectKey) {
        state.snapshots = [];
      ...
      state.snapshots = await getAllJSON('/api/snapshots?max_words=5000');
      state.worktrees = await getAllJSON('/api/worktrees?max_words=5000');
      renderSnapshotSelectors();
    ...
    async function renderSelected() {
      ...
      state.files = await getAllJSON(`/api/snapshots/${snapshot.id}/files?max_words=5000`);
    ...
    async function loadSourceLines(filePath, lines) {
      ...
      for (let start = 1; start <= end; start += 200) {
        requests.push(
          getAllSectionJSON(`/api/source-lines?snapshot_id=${state.selected.id}&file_path=${encodeURIComponent(filePath)}&start=${start}&end=${Math.min(end, start + 199)}&max_words=5000`, 'lines')
            .catch(() => [])
        );
      }
    ...
    async function inspectLine(lineNumber) {
      ...
      const history = await getJSON(
        `/api/line-history?file_path=${encodeURIComponent(state.selectedFile)}&line_number=${lineNumber}&repo_path=${encodeURIComponent(currentProject()?.repo_path || '')}&suite=${encodeURIComponent(state.selected.suite)}&max_words=5000`
      ).catch(() => []);
    ...
    async function loadFile(filePath, targetLine = null) {
      ...
      renderFiles();
      const payload = await getAllFileJSON(`/api/snapshots/${state.selected.id}/files/${encodeURIComponent(filePath).replaceAll('%2F', '/')}?max_words=5000`);
    ...
    async function loadScopedTrend() {
      ...
      const params = new URLSearchParams({
        repo_path: currentProject()?.repo_path || '',
        branch: reference,
        suite: state.selected.suite,
        limit: '200'
      });
      const points = await getAllJSON(`/api/trend?${params}&max_words=5000`);
    ...
    async function loadComparison(refreshInsightList = true, chooseDefault = false) {
      ...
      const baseUrl = `/api/compare?snapshot_id=${state.selected.id}&baseline_snapshot_id=${baselineId}&max_words=5000`;
      const result = await getJSON(`${baseUrl}&view=overview`);
      result.files = await getAllSectionJSON(`${baseUrl}&view=files`, 'files');
      result.changed_lines = await getAllSectionJSON(
        `/api/changed-lines?snapshot_id=${state.selected.id}&baseline_snapshot_id=${baselineId}&max_words=5000`,
        'lines'
      );
